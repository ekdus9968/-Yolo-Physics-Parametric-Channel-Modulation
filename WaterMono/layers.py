...
def depth_to_disp(depth):
    disp = 1/depth
    shape = disp.shape
    disp = disp.view(shape[0], -1)
    min_values, _ = torch.min(disp, dim=1, keepdim=True)
    max_values, _ = torch.max(disp, dim=1, keepdim=True)
    normalized_disp = (disp - min_values) / (max_values - min_values)
    return normalized_disp.view(shape).unsqueeze(1)
